...
% Initialize variables used to store in the code
max_audio_length = 0;
...
for i = 1:length(transmitter_Audios)
    ...
    disp("sampling frequency of " + transmitter_Audios(i) + " = " + Fs + " Hz"); % [1] display sampling freq
    %Fs = 44.1 khz
...
for i = 1:length(band_widths)-1
    current_sum = band_widths(i) + band_widths(i+1);
    if current_sum > max_sum
        max_sum = current_sum;
    end
end
k0 = BW_Of_chosen_Signal + (50000 - max_sum); % the biggest available frequency range for RF filter to use between two modulated consecutive signals
k = k0 - 0.1*k0; %0.1 is safe factor.
% Modulating
fo = 100000; 
n = (choose_channel-1);
delta_f = 50000;                                    
...
violation_1 = (2/Fs)*( fo + ( length(transmitter_Audios) - 1 ) * delta_f + BW_Of_last_Signal + k0);             % violation of neqyest which appears after generarting FDM signal.
violation_2 = (2/Fs)*( fo + ( length(transmitter_Audios) - 1 ) * delta_f + 25000 + BW_Of_chosen_Signal + 2000); % violation of neqyest which appears after second modulation from Mixer.
                                                                                                                % 2000 is a safe factor for IF filter to use.
...
if test ~= 1
    width = bandwidth2; % width of the RF filter should be bigger than our signal Bandwidth ( no precision for this filter paramerters because it works at very high frequncy )
    A_stop1 = 60; % Attenuation in the first stopband = 60 dB
    F_stop1 = (choose_channel-1)*50000+100000-k;      % Edge of the first  stopband = (fn-k) [We choosed high k value that filter can use before interacting with another signal as it is not high selective filter and not practical to decrease k]
    ...
    F_stop2 = (choose_channel-1)*50000+100000+k;      % Edge of the second stopband = (fn+k) making it symmetric is better
...
% IF stage
width = bandwidth2;            % the same as our signal base bandwidth.
A_stop1 = 60;                           % Attenuation in the first stopband = 60 dB
F_stop1 = IF-0.5*width-2000;            % Edge of the first  stopband (2000 is used as this filter is high selective and decreasing it makes operation  much slower due to higher filter order will be implemented )
...
width = BW_Of_chosen_Signal;
F_stop = width + 0.1 * (50000 - width); % Edge of the stopband ( we have good available range of frequencies to increase F_stop so F_stop = 2 * F_pass is good enough)
